---
title: "Exploratory Data Analysis of Anxiety Predictors"
...
double_variable_plot <- function(data, variable, var_type) {
  anxiety_var <- "Anxiety Level (1-10)"
  
  if (variable == anxiety_var) return(NULL)  # Skip self-comparison
  
  column <- data[[variable]]
  
  if (var_type == "Ordinal" || var_type == "Discrete") {
    # Heatmap of counts for Anxiety × Variable
    plot_data <- data %>% 
    ...
    ggplot(data, aes(x = as.factor(!!sym(anxiety_var)), y = !!sym(variable))) +
      geom_boxplot(color = 1, fill = "lightblue") +
      labs(x = anxiety_var) +
      theme_minimal()
    
  } else if (var_type == "Categorical (Few)") {
    ggplot(data, aes(x = as.factor(!!sym(anxiety_var)), fill = !!sym(variable))) +
      geom_bar(stat = "count", position = "fill", color = 1) +
      scale_x_discrete(limits = as.character(1:10)) +
      labs(x = anxiety_var, y = "Proportion") +
      theme_minimal()
    
  } else if (var_type == "Categorical (Many)") {
    ggplot(data, aes(x = !!sym(anxiety_var), color = !!sym(variable))) +
