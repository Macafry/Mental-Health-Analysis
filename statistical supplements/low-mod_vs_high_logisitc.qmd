---
title: "Predicting Low / Moderate Anxiety vs High Anxiety"
...
inter_model <- glm(AnxietyBinary ~ (Sleep.Hours + Therapy.Sessions + Diet.Quality + Stress.Level + Caffeine.Intake)^2, data = model_data, family = binomial)
...
interaction_results <- lapply(interaction_pairs, function(pair) {
  ...
  inter_model <- glm(extended_formula, data = model_data, family = binomial)

  stats <- model.comparison(main_model, inter_model)$statistics # take only inter_model row
...
augmented_data <- model_data %>%
  mutate(
    ...
    log_odds = predict(inter_model, type = "link")  # add logit prediction from interaction model
  )
```
