...
model_data <- data %>%
  mutate(AnxietyBinary = ifelse(`Anxiety Level (1-10)` <= 7, "Low/Moderate Anxiety", "High Anxiety")) %>% # binary anxiety
  select(-`Anxiety Level (1-10)`) %>% # Remove detailed anxiety
  mutate(across(where(is.character), as.factor)) %>%
  mutate(AnxietyBinary = factor(AnxietyBinary, levels = c("Low/Moderate Anxiety", "High Anxiety"))) # ensure Low/Mod is baseline
...
# Fit an initial model based on PFI heuristics
...
# Funtion that extracs AUC from a model
...
# Zipping model labels and AUC into a dataframe for display
...
outlier_data <- model_data %>%
  select(
    Therapy.Sessions, Stress.Level, Sleep.Hours,
    Caffeine.Intake, Diet.Quality, AnxietyBinary
  ) %>%
  mutate(
    PredictedProb = predict(initial_model, ., type = "response"),
    PredictedAnxiety = ifelse(PredictedProb >= 0.5, "High Anxiety", "Low/Moderate Anxiety"),
...
# Plot Linear Predictor (logit) against Pearson Residuals
p2 <- ggplot(outlier_data, aes(x = LinearPredictors, y = PearsonResiduals)) +
  ...
  geom_text(
    data = filter(outlier_data, abs(PearsonResiduals) > 5),
    aes(label = Row), vjust = -0.5, size = 4.5, color = "black"
  ) +
  labs(
    title = "Linear Predictor vs Pearson Residuals",
...
# Plot Leverage against Pearson Residuals
p3 <- ggplot(outlier_data, aes(x = Leverage, y = PearsonResiduals)) +
  geom_point(alpha = 0.6) +
  geom_hline(yintercept = c(-2, 2), linetype = "dashed", color = "blue") +
  geom_vline(xintercept = 2 * mean(outlier_data$Leverage), linetype = "dotted", color = "red") +
  geom_text(
    data = filter(outlier_data, abs(PearsonResiduals) > 10 | Leverage > 0.04),
    aes(label = Row), vjust = -0.5, size = 4.5, color = "black"
  ) +
  labs(
    title = "Leverage vs Pearson Residuals",
    x = "Leverage (Hat Values)",
    y = "Pearson Residuals"
  ) +
  theme_minimal()
...
```{r outlier_table}
# Systematically filter for "outliers" and display them
...
# Fit a new model without the top 30 most influential observations
model_trimmed <- glm(
...
# Create a dataframe with predictors as rows and each model coefficients for columns
# Also a ratio of coefficients for a more straightforward comparison
coef_comparison <- data.frame(
  Term = names(coef(initial_model)),
  Full_Model = coef(initial_model),
  Trimmed_Model = coef(model_trimmed),
  Ratio = coef(model_trimmed) / coef(initial_model)
)

print(coef_comparison)

# compute and compare the AUC of both models
# also compute the AUC for the trimmed model on the full dataset for a realistic comparison
...
check_logit_linearity <- function(model) {
  # Extract the models data and augment it with linear predicted values (logits)
  data <- model$data %>% rename_with(make.names)
  ...
  for (var in numeric_vars) {
    ...
    # Plot logits vs variable with loess line and regression line on top
    ...
    plot_list[[var]] <- p # add plot to list of plots
  }
...
# Predictor Variables: 2-way interaction Set
...
interaction_results <- lapply(interaction_pairs, function(pair) {
  # fit a model with all the additive variables + this specific interaction term
  ...
  # extract statistics when compared against the aditive model
  stats <- model.comparison(additive_model, interaction_model)$statistics

  # Take only interaction_model stats
  data.frame(
...
baseline_row <- data.frame(
  ...
  AIC = round(AIC(additive_model), 3),
  BIC = round(BIC(additive_model), 3),
  BayesFactor = NA,
  p_value = NA
)
...
```{r interaction_model}
# compare the chosen model with interaction terms against the additive model
...
auc_additive <- auc(model_data$AnxietyBinary, predict(additive_model, type = "response"))
auc_interaction <- auc(model_data$AnxietyBinary, predict(interaction_model, type = "response"))

cat("Additive model AUC:    ", round(auc_full, 4), "\n")
cat("Interaction model AUC: ", round(auc_interaction, 4), "\n")
```
...
# Create a matrix of Sleep slopes | Therapy and Diet
augmented_data %>%
...
# Create a matrix of Therapy slopes | Sleep and Diet
augmented_data %>%
...
# Create a matrix of Diet slopes | Therapy and Sleep
augmented_data %>%
...
ggplot(augmented_data, aes(x = Therapy.Sessions, y = log_odds)) +
  ...
  labs(
    title = "Interaction: Therapy Sessions",
    subtitle = "LOESS (blue) vs. Linear (dashed red)",
    x = "Therapy Sessions",
    y = "Predicted Log-Odds"
  ) +
...
ggplot(augmented_data, aes(x = Diet.Quality, y = log_odds)) +
  ...
  labs(
    title = "Interaction: Diet Quality",
    subtitle = "LOESS (blue) vs. Linear (dashed red)",
    x = "Diet Quality",
    y = "Predicted Log-Odds"
  ) +
...
# prepare data for a calibration plot using 200 quantiles
cal_data <- model_data %>%
...
# Calibration plot using 200 quantiles
...
# prepare data for a calibration plot using 20 bins of width of 5%
cal_data <- model_data %>%
  mutate(
    pred_prob = pred_prob,
...
# Calibration plot using 20 bins of width of 5%
...
# print expected change factors after a 1 unit increase
