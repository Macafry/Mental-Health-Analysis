...
model_data <- data %>%
  ...
  filter(`Anxiety Level (1-10)` <= 7) # Remove high Anxiety Levels
...
ordinal_model <- polr(
  ordered(Anxiety.Level) ~ Stress.Level + Sleep.Hours + Caffeine.Intake +
...
```{r print_cm_function}
# Custom function to print the important Confusion Matrix statistics
# The amount of details by class + the number of classes don't format well on the output
print_cm <- function(predicted_level, true_level) {
  # ensure levels are treated as Factors
  predicted_level <- as.factor(predicted_level)
  true_level <- as.factor(true_level)

  # Compute confusion Matrix
  cm <- confusionMatrix(predicted_level, true_level)
...
```{r within_one_accuracy_function}
# Function that calculates the percentage of levels predicted within one of the true level
# That is the percentage of (levels predicted - true level) that is {-1, 0, 1}
within_one_accuracy <- function(predicted_level, true_level) {
  # ensure levels are treated numerically
  predicted_level <- as.numeric(predicted_level)
  true_level <- as.numeric(true_level)

  mean(abs(predicted_level - true_level) <= 1)
}
```

```{r to_level_function}
# This function takes in the predicted anxiety of a continuous model
# It then transforms it into one of the seven classes
to_level <- function(predicted_level_continuous) {
  # Make it an integer
  predicted_level <- round(predicted_level_continuous, 0)

  # Ensure it falls between the proper bounds
  pmax(pmin(predicted_level, 7), 1)
}
```
...
# Display Results
print_cm(predict(ordinal_model), model_data$Anxiety.Level)

w1_accuracy <- within_one_accuracy(predict(ordinal_model), model_data$Anxiety.Level)
cat(paste0("\nCases predicted within one level of the true level: ", round(100 * w1_accuracy, 2), "%"))
```
...
# This function takes in the result of pomcheck along with some variable indicators
# It outputs a plot that helps assess the parallel slopes assumption on the logit scale
plot_pom <- function(variable, variable_label, variable_index, pom_data) {
  # Reformat the data to be ggplot-friendly
  # `pom_data[[variable_index]]` contains the slope data for that variable
  plot_data <- pom_data[[variable_index]] %>%
    pivot_longer( # ggplot works best with long format
      cols = starts_with("Anxiety.Level_>="),
      names_to = "Threshold",
      values_to = "EmpiricalLogit"
    ) %>%
    mutate( # Relabel the axis variables
      Threshold = str_replace(Threshold, "Anxiety\\.Level_>=(\\d)", "Anxiety ≥ \\1"),
      Group = as.numeric(!!sym(variable))
    ) %>%
    filter(Threshold != "Anxiety ≥ 1") # All observations are `Anxiety ≥ 1`, removing redundancy

  # extract the proper labels for the x axis
...
# Extract the data to help asses the parallel slopes assumption
pom_data <- pomcheck(
  Anxiety.Level ~ Stress.Level + Sleep.Hours + Caffeine.Intake +
    Therapy.Sessions + Family.History.of.Anxiety,
  data = mutate(model_data, Anxiety.Level = as.factor(Anxiety.Level))
)

# Dataframe containing variable metadata to simplify the calling of the `plot_pom` function
pom_df <- data.frame(
  names = c("Stress.Level", "Sleep.Hours", "Caffeine.Intake", "Therapy.Sessions"),
  labels = c("Stress Level", "Sleep Hours", "Caffeine Intake", "Therapy Sessions"),
  pom_index = 1:4
)
...
predicted_anxiety <- predict(linear_model) %>% to_level()

# Display Results
print_cm(predicted_anxiety, model_data$Anxiety.Level)

w1_accuracy <- within_one_accuracy(predicted_anxiety, model_data$Anxiety.Level)
cat(paste0("\nCases predicted within one level of the true level: ", round(100 * w1_accuracy, 2), "%"))
```
...
# Showing the comparison between the original model and
# a model with a variable removed for each variable in the model
...
# Extract residuals into dataframe
resid_df <- data.frame(Residuals = resid(linear_model))
...
# Preparing a new dataset with all the typical outlier metrics for each observation
...
# Plot Leverage against Cooks Distance
# with Residual Magnitudes as point sizes
p1 <- ggplot(outlier_data, aes(x = Leverage, y = Cook)) +
  ...
  geom_text(
    data = filter(outlier_data, Leverage > 0.03 | Cook > 0.01),
...
# Plot Predicted Anxiety against Studentized Residuals
p2 <- ggplot(outlier_data, aes(x = PredictedAnxiety, y = StudentizedResiduals)) +
  ...
  geom_text(
    data = filter(outlier_data, abs(StudentizedResiduals) > 3),
    aes(label = Row), vjust = -0.5, size = 4.5, color = "black"
  ) +
  labs(
    title = "Predicted Anxiety vs Studentized Residuals",
    x = "Predicted Anxiety Level",
...
p3 <- ggplot(outlier_data, aes(x = Leverage, y = StudentizedResiduals)) +
  ...
  geom_vline(xintercept = 2 * mean(outlier_data$Leverage), linetype = "dotted", color = "red") +
  geom_text(
    data = filter(outlier_data, abs(StudentizedResiduals) > 3 | Leverage > 0.04),
    aes(label = Row), vjust = -0.5, size = 4.5, color = "black"
  ) +
  labs(
...
p4 <- ggplot(outlier_data, aes(x = Row, y = Cook)) +
  ...
  geom_text(
    data = filter(outlier_data, Cook > 0.02),
    aes(label = Row), vjust = -0.5, size = 4.5, color = "black"
  ) +
  labs(
...
# Quarto's side by side panels didn't give enough control
...
check_linearity <- function(model) {
  # Extract the models data and augment it with the predicted values
  data <- model$model %>% rename_with(make.names)
  data$predicted_value <- predict(model)

  # Extract variables to iterate over
...
```{r additivity}
# Restate initial model with a more descriptive name for the sake of testing
additive_model <- lm(Anxiety.Level ~ Stress.Level + Sleep.Hours + Caffeine.Intake + Therapy.Sessions + Family.History.of.Anxiety, data = model_data)

# fit a model with all 2-way interactions
interaction_model <- lm(Anxiety.Level ~ (Stress.Level + Sleep.Hours + Caffeine.Intake + Therapy.Sessions + Family.History.of.Anxiety)^2, data = model_data)

# Test if ANY interaction is significant
...
# Plot showcasing the standard normality of the residuals
ggplot(data.frame(resid = resids), aes(x = resid)) +
  ...
  annotate("text",
    x = 0.55, y = 600, label = "±0.5 (Correct Prediction)",
    vjust = 1.5, hjust = 0, color = "darkblue", linewidth = 3.8
  ) +
  annotate("text",
    x = 1.55, y = 550, label = "±1.5 (Within-One Prediction)",
    vjust = 1.5, hjust = 0, color = "darkred", linewidth = 3.8
  ) +
  labs(
    title = "Distribution of Residuals",
    subtitle = paste0("Mean = ", round(resid_mean, 4), ", SD = ", round(resid_sd, 4)),
    x = "Residual",
    y = "Count"
  ) +
  theme_minimal()
...
# Fit model on just stress level and sleep
simple_model <- lm(Anxiety.Level ~ Stress.Level + Sleep.Hours, data = model_data)
summary(simple_model)

# Get predicted anxiety
predicted_anxiety <- predict(simple_model) %>% to_level()
